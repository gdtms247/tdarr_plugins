const details = () => ({
  ...
  Description:
    ...
    '- VALIDATES required streams (must contain at least 1 video and 1 audio stream, otherwise FAILS)\n' +
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  // ================================
  // 🚨 NEW: HARD VALIDATION BLOCK
  // ================================
  if (streams.length === 0) {
    response.processFile = false;
    response.infoLog += '❌ ERROR: No ffprobe stream data found. Failing file.\n';
    return response;
  }

  const videoStreams = streams.filter((s) => norm(s.codec_type) === 'video');
  const audioStreamsCheck = streams.filter((s) => norm(s.codec_type) === 'audio');

  if (videoStreams.length === 0) {
    response.processFile = false;
    response.infoLog += '❌ ERROR: No video stream detected. Failing file.\n';
    return response;
  }

  if (audioStreamsCheck.length === 0) {
    response.processFile = false;
    response.infoLog += '❌ ERROR: No audio stream detected. Failing file.\n';
    return response;
  }
  // ================================
