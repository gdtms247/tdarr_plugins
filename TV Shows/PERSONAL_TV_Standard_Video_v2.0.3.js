module.exports.details = function details() {
  return {
    id: 'PERSONAL_TV_Standard_Video',
    Stage: 'Pre-processing',
    Name: 'PERSONAL_TV_Standard_Video',
    Type: 'Video',
    Operation: 'Transcode',
    Description:
      'v2.0.3 - Improved resolution detection using width + height to properly classify scope content (e.g., 1482x618 as 1080p). Preserves higher quality thresholds.',
    Version: '2.0.3',
    Tags: 'personal,qsv,hevc,tv,standard',
  };
};
...
// ✅ NEW: improved resolution detection
...
module.exports.plugin = function plugin(file) {
  ...
  // ✅ SAME LOGIC — now based on corrected tier
  if (tier === 'SD') {
    threshold = 1800;
    targetBitrate = 1200;
    minBitrate = 1000;
  ...
  for (const s of file.ffProbeData?.streams || []) {
    ...
    if (s.codec_type === 'audio' && br > 0) {
      audio += br / 1000;
    }

    if (s.codec_type === 'subtitle' && br > 0) {
      subs += br / 1000;
    }
  }
  ...
  // 🚨 FORCE TRANSCODE IF NOT ALLOWED
